...
PyObject* PyNumpy_FromCUDA(PyObject* self, PyObject* args, PyObject* kwds)
{
    ...
    {
        PyErr_SetString(PyExc_Exception, LOG_PY_UTILS "cudaToNumpy() needs CUDA mapped memory as input (allocate with mapped=1)");
        return NULL;
    }
    ...
    // have it reference the capsule to manage its deletion
    Py_INCREF(capsule);
...
// Initialize NumPy (return a PyObject* so callers can detect failure)
PyObject* PyNumpy_ImportNumpy()
{
    if (_import_array() < 0) // sets a Python exception on failure
        return NULL;
    Py_RETURN_TRUE;          // NumPy initialized
}
...
bool PyNumpy_RegisterTypes(PyObject* module)
{
    ...
    PyObject* ok = PyNumpy_ImportNumpy();
    if (!ok)                // exception already set by _import_array()
        return false;
    Py_DECREF(ok);
    return true;
}

#else  // HAS_NUMPY not defined -------------------------------------------------
