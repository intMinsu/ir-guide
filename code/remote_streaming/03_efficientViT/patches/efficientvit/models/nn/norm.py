# efficientvit/models/nn/norm.py
...
# --- make Triton optional (falls back to PyTorch RMSNorm) ---
try:
    from efficientvit.models.nn.triton_rms_norm import TritonRMSNorm2dFunc as _TRITON_RMS_FUNC
    _HAS_TRITON = True
except Exception:
    _TRITON_RMS_FUNC = None
    _HAS_TRITON = False
# ------------------------------------------------------------
...
class TritonRMSNorm2d(nn.LayerNorm):
    ...
    If Triton is available, uses fused Triton kernel.
    Otherwise, uses a pure-PyTorch fallback:
        y = x / sqrt(mean(x^2, dim=C) + eps) * weight + bias
    """
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if _HAS_TRITON and _TRITON_RMS_FUNC is not None:
            return _TRITON_RMS_FUNC.apply(x, self.weight, self.bias, self.eps)
        # PyTorch fallback
...
def build_norm(name: str = "bn2d", num_features: Optional[int] = None, **kwargs) -> Optional[nn.Module]:
